...
class MatchPerspectiveTests(unittest.TestCase):
 def render(self, club, home, hg, ag, away, opponent):
  return leak_safe_question({'question_text':f'Who did {club} play in the high-scoring 2024-25 league match that finished {home} {hg}-{ag} {away}?','options_json':json.dumps([opponent,'Other A','Other B','Other C']),'correct_index':0,'semantic_key':'v3bank|test|high_scoring|match|opp'})
