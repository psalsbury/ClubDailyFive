...
<section class="club-grid" aria-label="Choose your club"><?php foreach($clubs as $c): ?><a class="club" data-slug="<?= h($c['slug']) ?>" data-name="<?= h($c['name']) ?>" data-logo="<?= h($c['logo_path']) ?>" href="/?club=<?= h($c['slug']) ?>"><img src="<?= h($c['logo_path']) ?>" alt="" width="42" height="42"><span class="club-name"><?= h($c['name']) ?></span></a><?php endforeach ?></section></div>
